#!/usr/bin/env python3
"""
Quick test of actual quantum vs classical performance with real implementations.
"""
...
from quantum_rerank.evaluation.industry_standard_evaluation import ClassicalBaselines
...
def test_quantum_vs_classical():
    """Test actual quantum vs classical performance."""
    
    print("🔬 REAL QUANTUM vs CLASSICAL PERFORMANCE TEST")
    print("=" * 60)
    ...
    classical_baselines = ClassicalBaselines()
    classical_baselines.initialize_bm25(documents)
    classical_baselines.initialize_sentence_bert()
    classical_baselines.initialize_cross_encoder()
        ...
        for i, query in enumerate(queries[:5]):  # Test on first 5 queries for speed
            ...
            try:
                if method_key == 'quantum_hybrid':
                    ...
                    # Test BM25
                    rankings = classical_baselines.rank_with_bm25(query, documents)
                    
                elif method_key == 'sentence_bert':
                    # Test Sentence-BERT
                    rankings = classical_baselines.rank_with_sentence_bert(query, documents)
                    
                elif method_key == 'cross_encoder':
                    # Test Cross-Encoder
                    rankings = classical_baselines.rank_with_cross_encoder(query, documents)
...
if __name__ == "__main__":
    results = test_quantum_vs_classical()